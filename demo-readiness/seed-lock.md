...
**Status: RE-LOCKED FINAL ✅ (2026-06-10, Fix Sprint 4 — Demo Polish phase complete).** The demo dataset is **deterministic** and **idempotent**. Re-running `pnpm seed` always produces the identical "Acme Corp" dataset documented below (on a FRESH database run `pnpm --filter @hrms/api db:push` first — two tables were added in Fix Sprint 3). This is the dataset behind the **100% green** demo-readiness survey (all allowed role×module tab-views OK; role-guarded cells assert the redirect and report **GUARDED** — see the survey baseline below).
...
> **Fix Sprint 4 (2026-06-10, FINAL) changes:** **5 completed `timer_sessions`** for emp01 (real projects/categories, 1 converted to a timesheet entry, durations net of pauses — never a running/paused timer); 2 of the 4 `analyticsReports` flagged `filters.sample: true` (Monthly Headcount Report, Leave Utilization Summary — render a "Sample" chip in the Report Builder). NOT seed data: the AI insight banners on the attendance/leave admin Reports tabs generate live one-liners via `AiCoreService` (OPENAI_API_KEY) — fail-soft `{insight: null}` with HTTP 200 when unconfigured/erroring, 30-min in-memory cache per org.
...
### Daily work logging
- **5** projects, **6** task categories, **20** project assignments; **400** timesheet entries (linked to projects/categories); **1** timesheet policy; **4** submissions (2 rejected/disputed + 2 pending approval from Sarah's team); **5** completed timer sessions for emp01 (1 converted)
