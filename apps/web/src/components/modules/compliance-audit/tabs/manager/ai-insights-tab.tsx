...
import { Loader2, Sparkles, Brain, AlertTriangle, ShieldAlert } from 'lucide-react';
...
const features = [
  {
    key: 'team-compliance-risk-score',
    label: 'Team Compliance Risk Score',
    desc: 'Assess the overall compliance risk across your team',
    endpoint: '/compliance-audit/ai/team-compliance-risk-score',
    icon: ShieldAlert,
  },
];
              ...
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-1.5">
                ...
                {typeof results[key].riskScore === 'number' && (
                  <p className="text-sm font-medium">
                    Team Risk Score:{' '}
                    <span className={`${(results[key].riskScore as number) > 70 ? 'text-red-500' : (results[key].riskScore as number) > 40 ? 'text-amber-500' : 'text-green-500'}`}>
                      {results[key].riskScore as number}/100
                    </span>
                  </p>
                )}
                {Array.isArray(results[key].riskFactors) &&
                  (results[key].riskFactors as Array<{ factor: string; score: number; description: string }>)
                    .slice(0, 4)
                    .map((f, i) => (
                      <p key={i} className="text-xs text-gray-600 flex items-start gap-1.5">
                        <span className={`mt-0.5 flex-shrink-0 ${f.score > 70 ? 'text-red-500' : f.score > 40 ? 'text-amber-500' : 'text-green-500'}`}>⚠</span>
