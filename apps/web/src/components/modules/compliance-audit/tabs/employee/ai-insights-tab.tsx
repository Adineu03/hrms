...
import { Loader2, Sparkles, Brain, AlertTriangle, GraduationCap } from 'lucide-react';

interface AiResult {
  guidance?: Array<{ topic: string; advice: string; priority: string }>;
  nextSteps?: string[];
  summary?: string;
...
const features = [
  {
    key: 'compliance-coach',
    label: 'Compliance Coach',
    desc: 'Get personalized compliance guidance and tips based on your profile',
    endpoint: '/compliance-audit/ai/compliance-coach',
    icon: GraduationCap,
  },
];
        ...
        {features.map(({ key, label, desc, endpoint, icon: Icon }) => (
          <div key={key} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            >
              ...
              {loading === key ? 'Analyzing...' : 'Get Guidance'}
            </button>
            {errors[key] && (
            ...
            {results[key] && !errors[key] && (
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-1.5">
                ...
                {Array.isArray(results[key].guidance) &&
                  (results[key].guidance as Array<{ topic: string; advice: string; priority: string }>)
                    .slice(0, 4)
                    .map((g, i) => (
                      <div key={i} className="text-xs text-gray-600">
                        <p className="font-medium text-[#2c2c2c]">{g.topic}</p>
                        <p className="mt-0.5">{g.advice}</p>
                      </div>
                    ))}
                {Array.isArray(results[key].nextSteps) &&
                  (results[key].nextSteps as string[]).slice(0, 3).map((step, i) => (
