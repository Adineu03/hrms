...
import { Loader2, Sparkles, Brain, AlertTriangle, BarChart2 } from 'lucide-react';

interface AiResult {
  teamMetrics?: Array<{ metric: string; value: string; trend: string }>;
  insights?: string[];
...
const features = [
  {
    key: 'demo-team-analytics',
    label: 'Demo Team Analytics',
    desc: 'AI-powered team analytics using synthetic demo data',
    endpoint: '/demo-company/ai/demo-team-analytics',
    icon: BarChart2,
  },
];
          ...
          <div key={key} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            {results[key] && !errors[key] && (
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-1.5">
                ...
                {Array.isArray(results[key].teamMetrics) &&
                  (results[key].teamMetrics as Array<{ metric: string; value: string; trend: string }>)
                    .slice(0, 4)
                    .map((m, i) => (
                      <p key={i} className="text-xs text-gray-600 flex items-start gap-1.5">
                        <span className="text-[#4F46E5] mt-0.5 flex-shrink-0">•</span>
                        <span><span className="font-medium">{m.metric}</span>: {m.value} ({m.trend})</span>
                      </p>
                    ))}
