...
import { Loader2, Sparkles, Brain, AlertTriangle, Lightbulb } from 'lucide-react';

interface AiResult {
  personalizedInsights?: Array<{ title: string; description: string; actionable: boolean }>;
...
const features = [
  {
    key: 'demo-ai-insights-showcase',
    label: 'Demo AI Insights Showcase',
    desc: 'Experience personalized AI insights as a demo employee',
    endpoint: '/demo-company/ai/demo-ai-insights-showcase',
    icon: Lightbulb,
  },
];
        ...
        {features.map(({ key, label, desc, endpoint, icon: Icon }) => (
          <div key={key} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            >
              ...
              {loading === key ? 'Generating...' : 'Show AI Insights'}
            </button>
            {errors[key] && (
            ...
            {results[key] && !errors[key] && (
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-1.5">
                ...
                {Array.isArray(results[key].personalizedInsights) &&
                  (results[key].personalizedInsights as Array<{ title: string; description: string; actionable: boolean }>)
                    .slice(0, 4)
                    .map((ins, i) => (
                      <div key={i} className="text-xs text-gray-600">
                        <p className="font-medium text-[#2c2c2c] flex items-center gap-1">
                          {ins.actionable && <span className="text-green-500">●</span>}
                          {ins.title}
                        </p>
                        <p className="mt-0.5 text-gray-500">{ins.description}</p>
                      </div>
                    ))}
