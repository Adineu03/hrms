...
import { Loader2, Sparkles, Brain, AlertTriangle, TrendingUp } from 'lucide-react';

interface AiResult {
  growthPaths?: Array<{ role: string; timeframe: string; probability: string; requiredSkills: string[] }>;
...
const features = [
  {
    key: 'career-growth-predictor',
    label: 'Career Growth Predictor',
    desc: 'Discover your likely career growth paths based on your skills and role',
    endpoint: '/workforce-planning/ai/career-growth-predictor',
    icon: TrendingUp,
  },
];
        ...
        {features.map(({ key, label, desc, endpoint, icon: Icon }) => (
          <div key={key} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            >
              ...
              {loading === key ? 'Analyzing...' : 'Predict My Growth'}
            </button>
            {errors[key] && (
            ...
            {results[key] && !errors[key] && (
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-1.5">
                ...
                {results[key].prediction && (
                  <p className="text-sm font-medium text-[#4F46E5]">{results[key].prediction as string}</p>
                )}
                {Array.isArray(results[key].growthPaths) &&
                  (results[key].growthPaths as Array<{ role: string; timeframe: string; probability: string }>)
                    .slice(0, 3)
                    .map((p, i) => (
                      <p key={i} className="text-xs text-gray-600 flex items-start gap-1.5">
                        <span className="text-[#4F46E5] mt-0.5 flex-shrink-0">•</span>
                        <span>
                          <span className="font-medium">{p.role}</span> — {p.timeframe}
                          <span className="text-gray-400 ml-1">({p.probability})</span>
                        </span>
                      </p>
                    ))}
                {Array.isArray(results[key].insights) &&
                  (results[key].insights as string[]).slice(0, 2).map((ins, i) => (
                    <p key={i} className="text-xs text-gray-600 flex items-start gap-1.5">
                      <span className="text-[#4F46E5] mt-0.5 flex-shrink-0">•</span>
                      {ins}
                    </p>
