...
import { Loader2, Sparkles, Brain, TrendingUp, AlertTriangle } from 'lucide-react';
...
const features = [
  {
    key: 'personal-growth-insights',
    label: 'Personal Growth Insights',
    desc: 'AI-generated quarterly growth summary: skills developed, goals achieved, feedback themes, and career progression probability.',
    endpoint: '/people-analytics/ai/personal-growth-insights',
    icon: TrendingUp,
  },
];
          ...
          <div key={key} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            {results[key] && !errors[key] && (
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-2">
                ...
                {typeof results[key].quarterlySummary === 'string' && (
                  <p className="text-sm text-[#2c2c2c]">{results[key].quarterlySummary as string}</p>
                )}
                {typeof results[key].careerProgressionProbability === 'number' && (
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-gray-500">Career Progression Probability:</span>
                    <span className="text-xs font-medium text-[#4F46E5]">{results[key].careerProgressionProbability}%</span>
                  </div>
                )}
                {Array.isArray(results[key].skillsDeveloped) &&
                  (results[key].skillsDeveloped as string[]).length > 0 && (
                    <div>
                      <p className="text-xs font-medium text-gray-700 mb-1">Skills Developed:</p>
                      {(results[key].skillsDeveloped as string[]).map((s, i) => (
                ...
                {Array.isArray(results[key].nextQuarterSuggestions) &&
                  (results[key].nextQuarterSuggestions as string[]).map((sug, i) => (
                    <p key={i} className="text-xs text-gray-600 flex items-start gap-1.5">
                      <span className="text-[#4F46E5] mt-0.5">↗</span>
                      {sug}
                    </p>
