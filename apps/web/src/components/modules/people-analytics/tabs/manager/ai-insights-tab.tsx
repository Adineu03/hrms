...
import { Loader2, Sparkles, Brain, TrendingUp, AlertTriangle, MessageSquare, Zap } from 'lucide-react';
...
const features = [
  {
    key: 'natural-language-analytics',
    label: 'Natural Language Analytics',
    desc: 'Ask any team question in plain English: "What\'s my team\'s attrition rate for the last 6 months?" — get instant results.',
    endpoint: '/people-analytics/ai/natural-language-analytics',
    icon: MessageSquare,
  },
  {
    key: 'predictive-team-insights',
    label: 'Predictive Team Insights',
    desc: 'Flight risk prediction per team member, performance trajectory for next cycle, and engagement trend forecast.',
    endpoint: '/people-analytics/ai/predictive-team-insights',
    icon: TrendingUp,
  },
  {
    key: 'anomaly-opportunity-spotter',
    label: 'Anomaly & Opportunity Spotter',
    desc: 'Surfaces insights you didn\'t ask for. Detects unusual patterns in attendance, engagement, and productivity.',
    endpoint: '/people-analytics/ai/anomaly-opportunity-spotter',
    icon: Zap,
  },
];
          ...
          <div key={key} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            {results[key] && !errors[key] && (
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-2">
                ...
                {typeof results[key].answer === 'string' && (
                  <p className="text-sm text-[#2c2c2c]">{results[key].answer as string}</p>
                )}
                ...
                {Array.isArray(results[key].followUpQuestions) &&
                  (results[key].followUpQuestions as string[]).map((q, i) => (
                    <p key={i} className="text-xs text-gray-500 italic flex items-start gap-1.5">
