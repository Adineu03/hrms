...
import { Loader2, Sparkles, Brain, TrendingUp, AlertTriangle, DollarSign, BarChart3, Activity } from 'lucide-react';
...
const features = [
  {
    key: 'utilization-optimizer',
    label: 'Utilization Optimizer',
    desc: 'Analyze team utilization and recommend workload rebalancing. Identify burnout risks and under-utilization.',
    endpoint: '/daily-work-logging/ai/utilization-optimizer',
    icon: TrendingUp,
  },
  {
    key: 'budget-burn-predictor',
    label: 'Budget Burn Predictor',
    desc: '"At current rate, Project X exhausts budget by March 15, 3 weeks early." Forecast depletion dates and suggest adjustments.',
    endpoint: '/daily-work-logging/ai/budget-burn-predictor',
    icon: DollarSign,
  },
  {
    key: 'work-pattern-analysis',
    label: 'Work Pattern Analysis',
    desc: '"Team spends 31% on meetings — 40% above company average." Identify time sinks and inefficiencies.',
    endpoint: '/daily-work-logging/ai/work-pattern-analysis',
    icon: BarChart3,
  },
  {
    key: 'entry-anomaly-detection',
    label: 'Entry Anomaly Detection',
    desc: 'Flag unusual entries: 14 hours on a 3-hour task, weekend logging, scope creep signals.',
    endpoint: '/daily-work-logging/ai/entry-anomaly-detection',
    icon: Activity,
  },
];
          ...
          <div key={key} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            {results[key] && !errors[key] && (
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-2">
                ...
                {Array.isArray(results[key].weeklyInsights) &&
                  (results[key].weeklyInsights as string[]).map((ins, i) => (
                    <p key={i} className="text-xs text-gray-600 flex items-start gap-1.5">
                      <span className="text-[#4F46E5] mt-0.5">↗</span>
                      {ins}
                    </p>
