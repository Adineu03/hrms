...
import { Loader2, Sparkles, Brain, Clock, Tag, Search, BarChart3, AlertTriangle } from 'lucide-react';
...
const features = [
  {
    key: 'auto-fill-timesheet',
    label: 'Auto-Fill Timesheet',
    desc: 'AI reconstructs your day from calendar events, Jira activity, Slack patterns, and app usage. Presents a complete draft for review.',
    endpoint: '/daily-work-logging/ai/auto-fill-timesheet',
    icon: Clock,
  },
  {
    key: 'smart-categorization',
    label: 'Smart Categorization',
    desc: 'Auto-tag meetings as billable/non-billable, map Jira tickets to projects, and categorize admin, training, and client time.',
    endpoint: '/daily-work-logging/ai/smart-categorization',
    icon: Tag,
  },
  {
    key: 'gap-detector',
    label: 'Gap Detector',
    desc: 'Notices unlogged hours between known activities and prompts you to fill them in. End-of-day check if total hours are below expected.',
    endpoint: '/daily-work-logging/ai/gap-detector',
    icon: Search,
  },
  {
    key: 'productivity-insights',
    label: 'Productivity Insights',
    desc: 'Weekly personal summary: meeting time %, deep work %, admin %, context switches, most productive day and time.',
    endpoint: '/daily-work-logging/ai/productivity-insights',
    icon: BarChart3,
  },
];
          ...
          <div key={key} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            {results[key] && !errors[key] && (
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-2">
                ...
                {typeof results[key].weekSummary === 'string' && (
                  <p className="text-sm text-[#2c2c2c]">{results[key].weekSummary as string}</p>
                )}
                {typeof results[key].totalLoggedHours === 'number' && (
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-1">
                      <span className="text-xs text-gray-500">Logged:</span>
                      <span className="text-xs font-medium text-[#4F46E5]">{results[key].totalLoggedHours}h</span>
                    </div>
                    {typeof results[key].expectedHours === 'number' && (
                      <div className="flex items-center gap-1">
                        <span className="text-xs text-gray-500">Expected:</span>
                        <span className="text-xs font-medium text-gray-700">{results[key].expectedHours}h</span>
                      </div>
                    )}
                  </div>
                )}
                {Array.isArray(results[key].gaps) && (results[key].gaps as Array<{ prompt: string }>).length > 0 && (
                  <div>
                    <p className="text-xs font-medium text-orange-600 mb-1">Unlogged Gaps Detected:</p>
                    {(results[key].gaps as Array<{ startTime: string; duration: number; prompt: string }>).map((gap, i) => (
