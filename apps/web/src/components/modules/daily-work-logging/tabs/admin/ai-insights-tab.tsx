...
import { Loader2, Sparkles, Brain, TrendingUp, AlertTriangle, DollarSign, Shield, Users } from 'lucide-react';
...
const features = [
  {
    key: 'utilization-intelligence',
    label: 'Utilization Intelligence',
    desc: 'Org-wide utilization by department, grade, location, project type. Benchmarked against industry standards.',
    endpoint: '/daily-work-logging/ai/utilization-intelligence',
    icon: TrendingUp,
  },
  {
    key: 'revenue-leakage-detector',
    label: 'Revenue Leakage Detector',
    desc: 'Identify billable work logged as non-billable. Quantify financial impact of categorization errors.',
    endpoint: '/daily-work-logging/ai/revenue-leakage-detector',
    icon: DollarSign,
  },
  {
    key: 'compliance-auto-check',
    label: 'Compliance Auto-Check',
    desc: 'Validate timesheets against labor laws across 50+ jurisdictions before payroll. Flag violations.',
    endpoint: '/daily-work-logging/ai/compliance-auto-check',
    icon: Shield,
  },
  {
    key: 'capacity-planner',
    label: 'Capacity Planner',
    desc: 'Model future capacity from project pipeline, attrition forecast, and hiring plan. Identify shortfalls.',
    endpoint: '/daily-work-logging/ai/capacity-planner',
    icon: Users,
  },
];
          ...
          <div key={key} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            {results[key] && !errors[key] && (
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-2">
                ...
                {typeof results[key].topInsight === 'string' && (
                  <p className="text-sm text-[#2c2c2c]">{results[key].topInsight as string}</p>
                )}
                {typeof results[key].prePayrollClearance === 'boolean' && (
                  <div className={`text-xs font-medium px-2 py-1 rounded ${results[key].prePayrollClearance ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                    Pre-Payroll Clearance: {results[key].prePayrollClearance ? 'Approved' : 'Issues Found'}
                  </div>
                )}
