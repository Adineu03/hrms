...
import { Loader2, Sparkles, Brain, AlertTriangle, Shield, BarChart, TrendingUp } from 'lucide-react';

interface AiResult {
  insights?: string[];
  suggestions?: Array<{ title: string; description: string; estimatedSaving: string; priority: string }>;
  recommendations?: string[];
  anomalies?: Array<{ description: string; severity: string; amount?: number }>;
  discrepancies?: Array<{ description: string; type: string; estimatedImpact: string }>;
  issues?: Array<{ regulation: string; status: string; riskLevel: string; action: string }>;
  summary?: string;
...
const features = [
  {
    ...
    desc: 'Detect unusual payroll patterns before processing',
    endpoint: '/payroll-processing/ai/anomaly-detection',
    icon: AlertTriangle,
  },
  {
    key: 'smart-reconciliation',
    label: 'Smart Reconciliation',
    desc: 'AI-assisted payroll reconciliation to find discrepancies',
    endpoint: '/payroll-processing/ai/smart-reconciliation',
    icon: TrendingUp,
  },
  {
    key: 'statutory-compliance-check',
    label: 'Statutory Compliance Checker',
    desc: 'Verify payroll statutory compliance across all regulations',
    endpoint: '/payroll-processing/ai/statutory-compliance-check',
    icon: Shield,
  },
  {
    key: 'salary-benchmark',
    label: 'Salary Benchmark',
    desc: 'Compare your salary structures against industry benchmarks',
    endpoint: '/payroll-processing/ai/salary-benchmark',
    icon: BarChart,
  },
];
              ...
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-1.5">
                ...
                {typeof results[key].complianceScore === 'number' && (
                  <p className="text-sm font-medium text-[#2c2c2c]">
                    Compliance Score: {results[key].complianceScore as number}%
                  </p>
                )}
                  ...
                  (results[key].anomalies as Array<{ description: string; severity: string }>)
                    ...
                    .map((a, i) => (
                      <p key={i} className="text-xs text-gray-600 flex items-start gap-1.5">
                        <span
                          className={`mt-0.5 flex-shrink-0 ${a.severity === 'high' ? 'text-red-500' : a.severity === 'medium' ? 'text-amber-500' : 'text-blue-500'}`}
                        >
                          ⚠
                        </span>
                        {a.description}
                ...
                {Array.isArray(results[key].discrepancies) &&
                  (
                    results[key].discrepancies as Array<{
                      description: string;
                    ...
                    .map((d, i) => (
                      <p key={i} className="text-xs text-gray-600 flex items-start gap-1.5">
                        ...
                        {d.description} — {d.estimatedImpact}
                      </p>
                    ))}
                {Array.isArray(results[key].issues) &&
                  (
                    results[key].issues as Array<{
                      regulation: string;
                      status: string;
                      action: string;
                    }>
