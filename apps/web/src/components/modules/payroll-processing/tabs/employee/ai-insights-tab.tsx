...
const features = [
  {
    key: 'tax-optimizer',
    label: 'Tax Optimizer',
    desc: 'Get personalized tax-saving suggestions based on your salary structure',
    endpoint: '/payroll-processing/ai/tax-optimizer',
    icon: TrendingUp,
  },
];
          ...
          <div key={key} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            {results[key] && !errors[key] && (
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-1.5">
                ...
                {results[key].totalPotentialSaving && (
                  <p className="text-sm font-medium text-green-600">
                    Potential savings: {results[key].totalPotentialSaving as string}
                  </p>
                )}
                {Array.isArray(results[key].suggestions) &&
                  (
                    results[key].suggestions as Array<{
                      title: string;
                      description: string;
                    ...
                    .map((s, i) => (
                      <div key={i} className="text-xs text-gray-600 flex items-start gap-1.5">
                        ...
                        <span>
                          ...
                          {s.estimatedSaving && (
                            <span className="text-green-600 ml-1">({s.estimatedSaving})</span>
                          )}
                        </span>
                      </div>
                    ))}
