...
interface AiResult {
  anomalies?: Array<{ description: string; severity: string; employeeRef: string }>;
  summary?: string;
...
const features = [
  {
    key: 'anomaly-alerts',
    label: 'Payroll Anomaly Alerts',
    desc: 'Detect unusual payroll amounts in your team before approval',
    endpoint: '/payroll-processing/ai/anomaly-alerts',
    icon: AlertTriangle,
  },
];
          ...
          <div key={key} className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            {results[key] && !errors[key] && (
              <div className="mt-4 p-3 bg-[#f5f5f0] rounded-lg space-y-1.5">
                ...
                {typeof results[key].alertCount === 'number' && (
                  <p className="text-xs text-amber-600 font-medium">
                    {results[key].alertCount as number} alert(s) detected
                  </p>
                )}
                {Array.isArray(results[key].anomalies) &&
                  (
                    results[key].anomalies as Array<{
                      description: string;
                      severity: string;
                      employeeRef: string;
                    }>
